...
Revised = "04.08.2026"  # Revision date for this file.
...
FORCE_TOOL_NUMBER = None  # <<< NEW >>> None = use library/TC-derived tool number
...
# *****************************************************************************
parser = argparse.ArgumentParser(prog="Workbee_Smart", add_help=False)
...
# <<< NEW >>> lets the caller force every tool change to output a fixed
# tool number, instead of whatever number the library/Tool Controller carries.
# Useful on single-tool, manual-tool-change machines.
parser.add_argument(
    "--force-tool-number",
    default="",
    help="Force all tool changes to output this tool number instead of the "
    "library-derived number, e.g. --force-tool-number=1",
)
parser.add_argument(
    "--bcnc",
    action="store_true",
...
def processArguments(argstring):
    ...
    try:
        ...
        # <<< NEW >>> pick up the forced tool number, if one was passed:
        if args.force_tool_number:
            FORCE_TOOL_NUMBER = args.force_tool_number
        if args.bcnc:
            OUTPUT_BCNC = True
        if args.no_bcnc:
            OUTPUT_BCNC = False
            ...
            if command in ("M6", "M06"):
                ...
                if OUTPUT_TOOL_CHANGE:
                    for line in TOOL_CHANGE.splitlines(True):
                        out += linenumber() + line + "\n"
                    outlist[0] = " "
                    # <<< NEW >>> use the forced tool number if one was passed
                    # via --force-tool-number, otherwise fall back to the
                    # library/Tool Controller-derived number as before:
                    if FORCE_TOOL_NUMBER:
                        outlist[-1] = "T" + str(FORCE_TOOL_NUMBER)
                    else:
